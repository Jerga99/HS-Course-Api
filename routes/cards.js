...
router.get('/classes/:cardClass', function(req, res) {
  ...
  const cardDeckCards = cards.filter(card => card.cardClass === cardClass.toUpperCase())
  if (cardDeckCards) {
...
router.get('/races/:race', function(req, res) {
  ...
  const cardDeckCards = cards.filter(card => card.race === race.toUpperCase())
  if (cardDeckCards) {
    return res.json(cardDeckCards)
  } else {
    return res.status(422).send({errors: {message: 'Card not found!'}})
...
router.get('/qualities/:quality', function(req, res) {
  ...
  const cardDeckCards = cards.filter(card => card.rarity === quality.toUpperCase())
  if (cardDeckCards) {
    return res.json(cardDeckCards)
  } else {
    return res.status(422).send({errors: {message: 'Card not found!'}})
...
router.get('/types/:type', function(req, res) {
  ...
  const cardDeckCards = cards.filter(card => card.type === type.toUpperCase())
  if (cardDeckCards) {
    return res.json(cardDeckCards)
  } else {
    return res.status(422).send({errors: {message: 'Card not found!'}})
...
router.get('/factions/:faction', function(req, res) {
  ...
  const cardDeckCards = cards.filter(card => card.faction === faction.toUpperCase())
  if (cardDeckCards) {
    return res.json(cardDeckCards)
  } else {
    return res.status(422).send({errors: {message: 'Card not found!'}})
